...
int CyBtldr_RunAction_Data(CyBtldr_Action action, NSData *data, CyBtldr_CommunicationsData* comm, CyBtldr_ProgressUpdate* update)
{
    ...
    err = CYRET_SUCCESS; // CyBtldr_OpenDataFile(file);
...
int CyBtldr_Program_Data(NSData *data, CyBtldr_CommunicationsData* comm, CyBtldr_ProgressUpdate* update)
{
    return CyBtldr_RunAction_Data(PROGRAM, data, comm, update);
}

void DataBootloader::load(NSData *data, void (*progress)(uint8_t, uint16_t))
{
    CyBtldr_CommunicationsData comms;
    int result = CyBtldr_Program_Data(
        &comms,
        progress);
