...
TargetPanel::getConfig() const
{
	...
	config.scsiId = scsiId.first & CONFIG_TARGET_ID_BITS;
...
void
TargetPanel::setConfig(const TargetConfig& config)
{
...
{
	return CtrlGetValue<uint8_t>(myScsiIdCtrl).first & CONFIG_TARGET_ID_BITS;
}
